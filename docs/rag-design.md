...
8. Return file paths, line numbers, snippets, scores, and explanations through the Agent or API.
...
`backend/app/rag/vector_store.py` implements a Chroma-backed vector store as the default Phase 2 backend. `ChromaVectorStore` stores supplied embeddings, source documents, and metadata under `VECTOR_STORE_PATH`, then returns Top-K chunks with file path, line range, content, and similarity score.

A lightweight `JsonVectorStore` remains available for deterministic unit tests and fallback debugging, but runtime retrieval defaults to Chroma through `VECTOR_STORE_BACKEND=chroma`.
...
`retrieve_code(project_path, query, top_k)` is registered in the Tool Registry. The Agent can call it when the user asks where logic is implemented or needs semantic code context.

Phase 3 also exposes retrieval through:

- `POST /api/projects/index`
- `POST /api/projects/search`

The tool and API both return references shaped for UI/API responses:
...
RAG validation includes scanner, chunker, embedding, vector store, retriever, Agent tool, and API search tests. Phase completion also runs the full backend suite with an isolated Chroma path so local vector-store state cannot hide regressions.
